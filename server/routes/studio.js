...
router.post('/video-prompt', async (req, res) => {
  ...
  try {
    ...
    const systemPrompt = `You are a professional cinematographer and AI video prompt engineer specializing in LTX-2 video generation. Your job is to write a single, flowing paragraph prompt that will generate stunning cinematic B-roll footage using LTX-2.
...
Write the complete LTX-2 prompt now:`;
    ...
    res.json({
      ...
      prompt,
      negativePrompt,
      metadata: { subject, mood, platform, shotStyle, duration, ltxPlaygroundUrl: 'https://app.ltx.studio/ltx-2-playground/t2v' }
    });
  } catch (error) {
...
Return ONLY the enhanced prompt. No explanation.`
    ...
    res.json({
      ...
      metadata: { width, height, aspectRatio: aspectRatio || '16:9', style: style || 'photorealistic', seed, provider: 'Pollinations AI (Free)' }
    });
  } catch (error) {
...
router.post('/generate-variations', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, variations, prompt: fullPrompt, metadata: { width, height, style } });
  } catch (error) {
...
router.post('/build-prompt', async (req, res) => {
  ...
  if (!contentTopic) return res.status(400).json({ error: 'Content topic is required' });
...
router.post('/storyboard', async (req, res) => {
  ...
  try {
    ...
    const systemPrompt = `You are a professional film director and storyboard artist who creates visual production plans for social media content creators. Your job is to break content into cinematic scenes with detailed visual descriptions that can be used to generate AI images and video B-roll.
...
Return a JSON object in this EXACT format:
{
  ...
  "scenes": [
    ...
    res.json({
      success: true,
      storyboard,
      metadata: { mode, platform, aspect, sceneCount: storyboard.scenes.length, ltxPlaygroundUrl: 'https://app.ltx.studio/ltx-2-playground/t2v' }
    });
  } catch (error) {
...
router.post('/storyboard/regenerate-scene', async (req, res) => {
  ...
  if (!scene) return res.status(400).json({ error: 'Scene data is required' });
