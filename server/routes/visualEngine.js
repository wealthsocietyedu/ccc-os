...
function logGen(userId, type, provider, credits, prompt, status, url, meta) {
  return db.prepare('INSERT INTO visual_generations (user_id,generation_type,provider,credits_used,prompt,status,result_url,metadata) VALUES (?,?,?,?,?,?,?,?)').run(userId, type, provider, credits, prompt, status, url||null, meta ? JSON.stringify(meta) : null);
}
...
router.post('/plan', requireAuth, async (req, res) => {
  ...
  try {
    const msg = await client.messages.create({ model:'claude-opus-4-5', max_tokens:800, messages:[{ role:'user', content:`You are an AI content director. Given this brief, generate optimized prompts for all content types.
...
router.post('/generate/image', requireAuth, async (req, res) => {
  ...
  const styleMap = { photorealistic:'photorealistic, 8k, professional photography, sharp focus', cinematic:'cinematic film still, dramatic lighting, movie quality, 4k', artistic:'digital art, concept art, artstation, highly detailed', minimal:'minimalist, clean, studio lighting, white background', dark:'dark moody aesthetic, dramatic shadows, noir, high contrast', vibrant:'vibrant saturated colors, bold composition, dynamic' };
  const dims = { '16:9':{w:1280,h:720}, '9:16':{w:720,h:1280}, '1:1':{w:1024,h:1024}, '4:3':{w:1024,h:768} };
...
router.post('/generate/faceless-video', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!script && topic) {
      const m = await client.messages.create({ model:'claude-opus-4-5', max_tokens:300, messages:[{ role:'user', content:`Write a 30-second faceless video script for: "${topic}". Platform: ${platform||'Reels'}. Style: ${visualStyle||'motivational'}. Requirements: strong hook first 3 seconds, clear value, strong CTA, no "I"/"me", under 80 words. Return ONLY the script.` }] });
...
router.post('/generate/carousel', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!slides) {
      const m = await client.messages.create({ model:'claude-opus-4-5', max_tokens:400, messages:[{ role:'user', content:`Create viral carousel for: "${topic}". Niche: ${niche||'lifestyle'}. Platform: ${platform||'Instagram'}. Return ONLY valid JSON array of 6 slides: [{"headline":"...","body":"1-2 sentences","type":"hook|value|cta"}]` }] });
...
router.post('/generate/broll', requireAuth, async (req, res) => {
  ...
  if (credits.balance < cost) return res.status(402).json({ error:'Insufficient credits', balance:credits.balance, required:cost });
  try {
    const msg = await client.messages.create({ model:'claude-opus-4-5', max_tokens:350, system:'You are a cinematographer writing LTX-2 video prompts. Write a single flowing paragraph under 200 words. Start with the main action. Include camera work and lighting. Return ONLY the prompt text.', messages:[{ role:'user', content:`LTX-2 B-roll prompt for:\nSubject: ${subject}\nMood: ${mood||'cinematic'}\nPlatform: ${platform||'youtube'}\nShot: ${shotStyle||'cinematic'}\nDuration: ${duration||8} seconds` }] });
...
router.get('/status/:jobId', requireAuth, async (req, res) => {
  ...
  try {
    const r = await fetch(`https://api.blotato.com/v1/jobs/${req.params.jobId}`, { headers:{ 'Authorization':`Bearer ${process.env.BLOTATO_API_KEY}` } });
    const d = await r.json();
